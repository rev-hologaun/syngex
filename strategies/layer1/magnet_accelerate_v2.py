...
logger = logging.getLogger("Syngex.Strategies.MagnetAccelerateV2")
...
STOP_PCT_PHASE_1_LONG_V2 = 0.015   # v2: wider LONG Phase 1 stop (1% → 1.5%) — reduces whipsaw losses at 11.3% WR


class MagnetAccelerateV2(BaseStrategy):
    """
    ...
    strategy_id = "magnet_accelerate_v2"
    layer = "layer1"

    # v2: Track magnet at entry for dynamic target recalculation
    _entry_magnet_strike = None  # magnet strike when signal was created
    _current_magnet_strike = None  # magnet strike at current evaluation tick
    ...
    def evaluate(self, data: Dict[str, Any]) -> List[Signal]:
        ...
        # v2: Store magnet strike at current tick for dynamic target updates
        self._current_magnet_strike = magnet_strike
    ...
    def _phase1_pull(
        ...
        # v2: Store magnet strike at entry for dynamic target recalculation
        self._entry_magnet_strike = magnet_strike

        # Target: magnet strike (exit within 0.3% of it)
        target = magnet_strike

        # v2: Dynamic magnet target — recalculate if magnet shifted favorably
        if self._entry_magnet_strike is not None and self._current_magnet_strike is not None:
            original_target = self._entry_magnet_strike
            current_magnet = self._current_magnet_strike
            if direction == Direction.LONG and current_magnet > price:
                # Magnet moved higher — favorable for LONG pull; cap at new magnet
                target = min(original_target, current_magnet)
            elif direction == Direction.SHORT and current_magnet < price:
                # Magnet moved lower — favorable for SHORT pull; cap at new magnet
                target = max(original_target, current_magnet)

        # Stop: wider for LONG, unchanged for SHORT
        if direction == Direction.LONG:
            stop = price * (1 - STOP_PCT_PHASE_1_LONG_V2)  # v2: 1.5% instead of 1%
        else:
            stop = price * (1 + 0.01)
