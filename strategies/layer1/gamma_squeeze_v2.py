...
logger = logging.getLogger("Syngex.Strategies.GammaSqueezeV2")
...
class GammaSqueezeV2(BaseStrategy):
    """
    ...
    def _regime_passes(self, regime: str, wall_gex: float, gex_calc: Any) -> bool:
        ...
        POSITIVE regime: only fire if wall GEX is MASSIVE (>80th percentile).
        ...
        if regime == "POSITIVE":
            # Get all walls and compute 80th percentile
            walls = gex_calc.get_gamma_walls(threshold=MIN_WALL_GEX)
            if walls:
                gex_values = [abs(w["gex"]) for w in walls]
                gex_values.sort()
                p80_idx = max(0, int(len(gex_values) * 0.80) - 1)
                p80_gex = gex_values[p80_idx]
                if abs(wall_gex) >= p80_gex:
    ...
    def _enter_squeeze(
        ...
        if direction == Direction.SHORT and net_gamma >= 0:
            net_gamma_penalty *= 0.5
        # Apply penalty to confidence later (after _squeeze_confidence returns)
        ...
        # Apply net gamma direction penalty
        confidence *= net_gamma_penalty
