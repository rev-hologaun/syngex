...
logger = logging.getLogger("Syngex.Strategies.GEXDivergenceV2")
...
class GEXDivergenceV2(BaseStrategy):
    """
    ...
    def evaluate(self, data: Dict[str, Any]) -> List[Signal]:
        ...
        # Slope smoothness — downgrades jagged/noisy trends
        smoothness = self._check_slope_smoothness(price_window, gamma_window)
        confidence *= smoothness

        # Afternoon session boost — strategy shows highest WR at 50-59% conf in afternoon
        try:
    ...
    def _check_slope_smoothness(
        self, price_window: Any, gamma_window: Any
    ) -> float:
        """
        Measure how smooth (monotonic) the price/GEX slope is over the
        rolling window. Returns a score between 0 (jagged/noisy) and 1
        (clean trend).

        Formula: smoothness = max(0, 1.0 - (std_of_deltas / mean_abs_price))
        — essentially the inverse of coefficient of variation.
        """
        try:
            def _window_smoothness(values: List[float]) -> float:
                if len(values) < 3:
                    return 1.0  # not enough data to judge — assume clean
                deltas = [
                    values[i + 1] - values[i]
                ...
                return max(0.0, 1.0 - (std_dev / mean_abs))

            price_smooth = _window_smoothness(list(price_window.values)) if price_window.values else 1.0
            gamma_smooth = _window_smoothness(list(gamma_window.values)) if gamma_window.values else 1.0
            return (price_smooth + gamma_smooth) / 2.0
        except Exception:
            return 1.0  # on error, don't penalize
    ...
    def _check_confirmation(
        ...
        if divergence_type == "bearish":
            ...
            # 2× confirmation threshold = 0.4% (tightened from 0.6%)
            return price_change < CONFIRMATION_CANDLE_PCT * 2
        else:
