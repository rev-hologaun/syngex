...
logger = logging.getLogger("Syngex.Strategies.GEXImbalanceV2")

# ---------------------------------------------------------------------------
# Constants
# ---------------------------------------------------------------------------

PUT_HEAVY_RATIO = 0.55        # < 0.55 → long bias (raised from 0.45 to reduce marginal signal flood)
...
LONG_MIN_GEX_MAGNITUDE = 50000  # v2: minimum |net_gex_imbalance| for LONG — filters noise tail
...
class GEXImbalanceV2(BaseStrategy):
    """
    ...
    def _classify_bias(
        ...
        # Put-heavy → LONG bias (v2: rolling percentile adaptive threshold)
        if ratio < PUT_HEAVY_RATIO:
            # v2: Magnitude gate for LONG — filter noise tail where few signals fire
            net_imbalance = put_gex - call_gex  # positive when put-heavy
            if net_imbalance < LONG_MIN_GEX_MAGNITUDE:
                return (None, 0.0)
            # v2: Rolling percentile check — only fire if ratio is unusually low vs recent history
            passed, rank = self._rolling_percentile_threshold(ratio, "_ratio_history_pct")
            self._update_rolling_history("_ratio_history_pct", ratio)
            if not passed:
                return (None, 0.0)  # not extreme enough relative to recent activity
            # Strength: how far below threshold
            strength = min(1.0, (PUT_HEAVY_RATIO - ratio) / PUT_HEAVY_RATIO) * max(rank, 0.5)
            return ("LONG", strength)

        # Call-heavy → SHORT bias (v2: rolling percentile adaptive threshold)
        if ratio > CALL_HEAVY_RATIO:
            # v2: Rolling percentile check
            passed, rank = self._rolling_percentile_threshold(ratio, "_ratio_history_pct_shr", upper_pct=0.85)
            self._update_rolling_history("_ratio_history_pct_shr", ratio)
            if not passed:
                return (None, 0.0)
            # Normalize against realistic upper bound (3.0 = calls 3x puts)
            strength = min(1.0, (ratio - CALL_HEAVY_RATIO) / (3.0 - CALL_HEAVY_RATIO)) * max(rank, 0.5)
    ...
    # ------------------------------------------------------------------
    # Rolling percentile thresholds (v2: adaptive per-session)
    # ------------------------------------------------------------------
    PERCENTILE_WINDOW = 200  # recent data points for percentile calc

    def _rolling_percentile_threshold(self, current_value: float, history_attr: str, upper_pct: float = 0.85) -> tuple:
        """
        Check if current_value exceeds rolling percentile threshold.
        Returns (passed: bool, raw_pval: float).
        ...
        p_idx = int(len(sorted_vals) * upper_pct) - 1
        p_idx = max(0, min(p_idx, len(sorted_vals) - 1))
        threshold = sorted_vals[p_idx]
        passed = current_value >= threshold
        # Return percentile rank (0-1) of current_value within history
        rank = sum(1 for v in sorted_vals if v <= current_value) / len(sorted_vals)
        return (passed, rank)

    def _update_rolling_history(self, history_attr: str, value: float) -> None:
        """Append value to rolling history, capped at PERCENTILE_WINDOW."""
        vals = getattr(self, history_attr, [])
        vals.append(value)
        if len(vals) > self.PERCENTILE_WINDOW:
            vals = vals[-self.PERCENTILE_WINDOW:]
        setattr(self, history_attr, vals)
