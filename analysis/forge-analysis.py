...
OUTPUT_FILE = LOG_DIR / "strategy_analysis_report.md"
...
def generate_report(all_signals, strategy_results):
    ...
    lines.append(f"|--------|-------|")
    lines.append(f"| Total Resolved Signals | {len(all_signals):,} |")
    lines.append(f"| Total Wins | {total_wins:,} |")
    lines.append(f"| Total Losses | {total_losses:,} |")
    lines.append(f"| Time-Expired (CLOSED) | {total_closed:,} |")
    lines.append(f"| Overall Win Rate (excl. CLOSED) | {win_rate(total_wins, total_losses, total_closed):.1f}% |")
    lines.append(f"| Total P&L | ${fmt_num_no_comma(total_pnl, 2)} |")
    lines.append(f"| Avg P&L per Signal | ${fmt_num_no_comma(avg_pnl([s.get('pnl',0) for s in all_signals]))} |")
    lines.append(f"| Symbols Traded | {', '.join(sorted(set(s['_symbol'] for s in all_signals)))} |")
    lines.append("")
    ...
    for sid in sorted(strategy_results.keys()):
        ...
        lines.append(f"### {sid}")
        lines.append("")
        lines.append(f"**Symbols:** {', '.join(sorted(r['symbols']))}  |  **Total Signals:** {total:,}  |  **Win Rate:** {wr:.1f}%  |  **Avg P&L:** ${fmt_num(avg_p)}  |  **Avg Hold:** {avg_h:.0f}s ({avg_h/60:.1f}m)  |  **Median Hold:** {med_h:.0f}s")
        lines.append("")
        
        # ── 1. Confidence Level Performance ──────────────────────
        lines.append("#### 1) Performance by Confidence Level")
        lines.append("")
        lines.append("| Confidence Bucket | Total | Wins | Losses | Closed | Win Rate | Avg P&L | Avg P&L% |")
        lines.append("|---|---|---|---|---|---|---|---|")
        ...
        for label, lo, hi in CONFIDENCE_BUCKETS:
            ...
            if cb and cb["total"] > 0:
                ...
                lines.append(f"| {label} | {cb['total']} | {cb['wins']} | {cb['losses']} | {cb['closed']} | {cwr:.1f}% | ${fmt_num(cap)} | {cpp:.1f}% |")
        ...
        lines.append("")
        lines.append("| Market Type | Total | Wins | Losses | Closed | Win Rate | Avg P&L |")
        lines.append("|---|---|---|---|---|---|---|")
        ...
        for mk in sorted(r["trend_perf"].keys()):
            ...
            if tp["total"] > 0:
                ...
                tap = tp["pnl"] / tp["total"]
                lines.append(f"| {mk} | {tp['total']} | {tp['wins']} | {tp['losses']} | {tp['closed']} | {twr:.1f}% | ${fmt_num(tap)} |")
        
        lines.append("")
        lines.append("**Regime Performance:**")
        lines.append("")
        lines.append("| Regime | Total | Wins | Losses | Closed | Win Rate | Avg P&L |")
        lines.append("|---|---|---|---|---|---|---|")
        ...
        for rk in sorted(r["regime_perf"].keys()):
            ...
            if rp["total"] > 0:
                ...
                rap = rp["pnl"] / rp["total"]
                lines.append(f"| {rk} | {rp['total']} | {rp['wins']} | {rp['losses']} | {rp['closed']} | {rwr:.1f}% | ${fmt_num(rap)} |")
        ...
        lines.append("")
        lines.append("| Timeframe | Total | Wins | Losses | Closed | Win Rate | Avg P&L |")
        lines.append("|---|---|---|---|---|---|---|")
        ...
        for tk in sorted(r["time_window_perf"].keys()):
            ...
            if twp["total"] > 0:
                ...
                lines.append(f"| {tk} | {twp['total']} | {twp['wins']} | {twp['losses']} | {twp['closed']} | {twr:.1f}% | ${fmt_num(tap)} |")
        ...
        lines.append("")
        lines.append("| Direction | Total | Wins | Losses | Closed | Win Rate | Avg P&L |")
        lines.append("|---|---|---|---|---|---|---|")
        ...
        for dk in sorted(r["direction_perf"].keys()):
            ...
            if dp["total"] > 0:
                ...
                dap = dp["pnl"] / dp["total"]
                lines.append(f"| {dk} | {dp['total']} | {dp['wins']} | {dp['losses']} | {dp['closed']} | {dwr:.1f}% | ${fmt_num(dap)} |")
        ...
        lines.append("")
        lines.append("| Hold Time | Total | Wins | Losses | Closed | Win Rate | Avg P&L |")
        lines.append("|---|---|---|---|---|---|---|")
        ...
        for hk in sorted(r["hold_time_buckets"].keys()):
            ...
            if htb["total"] > 0:
                ...
                lines.append(f"| {hk} | {htb['total']} | {htb['wins']} | {htb['losses']} | {htb['closed']} | {hwr:.1f}% | ${fmt_num(hap)} |")
    ...
    lines.append("")
    lines.append("| Rank | Strategy | Signals | Win Rate | Avg P&L | Best Confidence | Best Market Type | Best Timeframe |")
    lines.append("|---|---|---|---|---|---|---|---|")
    ...
    ranked.sort(key=lambda x: x[3], reverse=True)  # Sort by avg P&L
    
    for i, (sid, total, wr, avg_p, best_cb, best_mt, best_tf) in enumerate(ranked, 1):
        lines.append(f"| {i} | {sid} | {total:,} | {wr:.1f}% | ${fmt_num(avg_p)} | {best_cb or 'N/A'} | {best_mt or 'N/A'} | {best_tf or 'N/A'} |")
