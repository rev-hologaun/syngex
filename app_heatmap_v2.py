...
def _load_stats_from_disk(symbol: str) -> Dict[str, RunningStats]:
    ...
    for line in log_path.read_text().strip().splitlines():
        ...
        try:
            ...
            if not sid or not sid.endswith("_v2"):
                continue
            if sid not in stats:
...
@app.route("/")
def index():
    """Serve the v2 heatmap HTML page."""
    try:
        return render_template("heatmap.html", symbol=SYMBOL, port=PORT, v2=True)
    except Exception:
...
if __name__ == "__main__":
    logger.info("Syngex V2 Heatmap Dashboard starting on port %d", PORT)
